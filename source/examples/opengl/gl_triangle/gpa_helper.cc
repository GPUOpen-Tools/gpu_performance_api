...
bool GPAHelper::SetUpGPA()
{
    ...
    m_counterFileName = std::string(utf8ExecutablePath.begin(), utf8ExecutablePath.end()).append(m_counterFileName);
    m_gpaLogFileName  = std::string(utf8ExecutablePath.begin(), utf8ExecutablePath.end()).append(m_gpaLogFileName);
                ...
                for (gpa_uint32 i = 0; i < numEnabledCounters; i++)
                {
                    ...
                    if (verify)
                    {
                        ...
                        if (GPA_USAGE_TYPE_PERCENTAGE == usageType)
                        {
                            ...
                            {
                                errorString << pCounterName << " Counter Value is " << fResult << ". Expected counter value to be in interval [0.0f,100.0f]";
                        ...
                        if (counterName == "GPUTime")
                        {
                            ...
                            if (!verificationSuccess)
                            {
                                errorString << "GPUTime. Counter Value is " << fResult << ". Expected counter value to be greater than 0.0f";
                                GPALogger(GPA_LOGGING_ERROR, errorString.str().c_str());
                            }
                        }
                        else if (counterName == "GPUBusy")
                        {
                            ...
                            if (!verificationSuccess)
                            {
                                errorString << "GPUBusy. Counter Value is " << fResult << ". Expected counter value to be in interval [0.0f,100.0f]";
                                GPALogger(GPA_LOGGING_ERROR, errorString.str().c_str());
                            }
                        }
                        ...
                        /*
                        GPA-619: GPA is reporting incorrect values for these counters
                        else if (counterName == "PSPixelsOut")
                        {
                            verificationSuccess &= (fResult == 70125);

                            if (!verificationSuccess)
                            {
                                errorString << "PSPixelsOut. Counter Value is " << fResult
                                            << ". Expected counter value to be equal to 70125";
                                GPALogger(GPA_LOGGING_ERROR, errorString.str().c_str());
                            }
                        }
                        else if (counterName == "PrimitivesIn")
                        {
                            verificationSuccess &= (fResult == 1);

                            if (!verificationSuccess)
                            {
                                errorString << "PrimitivesIn. Counter Value is " << fResult
                                            << ". Expected counter value to be equal to 1";
                                GPALogger(GPA_LOGGING_ERROR, errorString.str().c_str());
                            }
                        }
                        else if (counterName == "VSVerticesIn")
                        {
                            verificationSuccess &= (fResult == 3);

                            if (!verificationSuccess)
                            {
                                errorString << ". VSVerticesIn. Counter Value is " << fResult
                                            << ". Expected counter value to be equal to 3";
                                GPALogger(GPA_LOGGING_ERROR, errorString.str().c_str());
                            }
                        }*/
                    }
