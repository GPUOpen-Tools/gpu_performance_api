...
public:
    ...
    clPerfCounterBlock(cl_device_id clDevice, cl_ulong blockID, cl_uint maxActive, std::vector<cl_ulong> pCounters);
    ...
    {
        return (passID * m_maxActive <= m_pCounters.size() && nullptr != m_pclCounters) ? &m_pclCounters[passID * m_maxActive] : nullptr;
    }
    ...
    {
        return (cl_uint)m_pCounters.size();
    }
    ...
    {
        return (cl_uint)(ceilf((float)m_pCounters.size() / m_maxActive));
    }
    ...
    cl_ulong GetResult(cl_ulong counterID)
    {
        return m_results[counterID];
    }
