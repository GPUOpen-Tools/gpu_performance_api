...
enum AmdExtTTWaveStartExt : UINT32
{
    ...
    ExtWgId  = 0x00000003  ///< Wavestart tokens extended with workgroup (WG) ID and flags for dynamic VGPRs and last wave in WG. TT 3.5.
};
    ...
    struct
    {
        union
        {
            struct
            {
                ...
                UINT32 stallMode                    : 2;  ///< Describes behavior when buffer full
                UINT32 stallAllSimds                : 1;  ///< Stall all SIMDs for thread trace stall.
                UINT32 excludeNonDetailShaderData   : 1;  ///< Only emit shader tokens from the SIMD that have been
                                                          ///  selected for detail instruction tracing
                UINT32 enableExecPopTokens          : 1;  ///< Output exec tokens
                AmdExtTTWaveStartExt waveStartExt   : 2;  ///< Configure the wave start token extension mode
