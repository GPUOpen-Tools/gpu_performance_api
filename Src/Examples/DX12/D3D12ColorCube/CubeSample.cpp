...
void LogGPA(GPA_Logging_Type loggingType, const char* logMessage)
{
    ...
    CubeSample::Instance()->GPA_Log(logMessage);
}
...
)
{
    switch (uMsg)
    {
        case WM_SHOWWINDOW:
        {
            ...
            CubeSample::Instance()->GPA_InitializeAndOpenContext();
            CubeSample::Instance()->GPA_PopulateAvailableCounterInfo();
            return 0;
        }

        case WM_PAINT:
        {
            CubeSample::Instance()->GPA_CreateProfilingSession();
            CubeSample::Instance()->GPA_EnableAllCounters();
            CubeSample::Instance()->GPA_BeginProfilingSession();

            do
            {
                CubeSample::Instance()->GPA_BeginPass();
                CubeSample::Instance()->Draw();  // draw the cube
                CubeSample::Instance()->GPA_EndPass();
            }
            while (CubeSample::Instance()->GPA_NextPassNeeded());

            CubeSample::Instance()->GPA_EndProfilingSession();
...
void CubeSample::Init()
{
    ...
    m_executablePath = std::wstring(moduleString.begin(), moduleString.begin() + (lastSlashPosition + 1));
...
bool CubeSample::InitializeCommonResource()
{
    ...
    /// Enable Debug layer
#ifdef _DEBUG
    result = D3D12GetDebugInterface(__uuidof(ID3D12Debug),
                                    reinterpret_cast<void**>(&m_pd3d12DebugInterface));
    ...
    {
        decltype(DXGIGetDebugInterface)* dxgiDebugFunc = reinterpret_cast<decltype(DXGIGetDebugInterface)*>(GetProcAddress(dxgiHandle, "DXGIGetDebugInterface"));
    ...
    result = CreateDXGIFactory2(dxgiFactoryFlags, __uuidof(IDXGIFactory2),
    ...
    if (!dxgiAdapters.empty())
    {
        ...
        // Create the device
        result = D3D12CreateDevice(pdxgiAdapter1, D3D_FEATURE_LEVEL_12_0,
        ...
        result = m_pd3d12Device->CreateCommandQueue(&cmdQueueDesc,
                                                    __uuidof(ID3D12CommandQueue),
                                                    reinterpret_cast<void**>(&m_pd3d12CommandQueue));
        ...
        swapChainDesc.Width = static_cast<UINT>(g_windowWidth);
        swapChainDesc.Height = static_cast<UINT>(g_windowHeight);
        ...
        result = m_pdxgiFactory2->CreateSwapChainForHwnd(m_pd3d12CommandQueue,
                                                         g_windowHandle,
                                                         &swapChainDesc,
                                                         nullptr,
                                                         nullptr,
        ...
        result = pdxgiSwapChain1->QueryInterface(__uuidof(IDXGISwapChain3),
                                                 reinterpret_cast<void**>(&m_pdxgiSwapChain3));
        ...
        result = m_pd3d12Device->CreateDescriptorHeap(&rtvDescriptionHeap,
                                                      __uuidof(ID3D12DescriptorHeap),
                                                      reinterpret_cast<void**>(&m_pd3d12DescriptorHeap));
        ...
        result = m_pd3d12Device->CreateFence(0,
                                             D3D12_FENCE_FLAG_NONE,
                                             __uuidof(ID3D12Fence),
                                             reinterpret_cast<void**>(&m_pd3d12Fence));
        ...
        if (nullptr != m_fenceEvent)
        {
            ...
            std::wstring shaderPath = m_executablePath;
            ...
            result = m_pd3d12Device->CreateGraphicsPipelineState(&psoDesc,
                                                                 __uuidof(ID3D12PipelineState),
                                                                 reinterpret_cast<void**>(&m_pfillDrawPipeline));
            ...
            result = m_pd3d12Device->CreateGraphicsPipelineState(&psoDesc,
                                                                 __uuidof(ID3D12PipelineState),
                                                                 reinterpret_cast<void**>(&m_pWireframePipeline));
...
{
    return m_topLeftViewport.Init() &&
           m_topRightViewport.Init() &&
           m_bottomLeftViewport.Init() &&
           m_bottomRightViewport.Init();
}
...
CubeSample::TopLeftViewport::TopLeftViewport() : Viewport()
{
...
bool CubeSample::TopLeftViewport::Init()
{
    if (m_viewportInitialized)
    {
        ...
        result = ms_pCubeSample->m_pd3d12Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                                        __uuidof(ID3D12CommandAllocator),
                                                                        reinterpret_cast<void**>(&m_pd3d12DirectCmdListAllocator));
...
void CubeSample::TopLeftViewport::Draw()
{
    ...
    ms_pCubeSample->GPA_BeginSample(m_gpaCommandListId, m_sampleId);                                // Start sample on the command list
    #pragma endregion
...
bool CubeSample::TopRightViewport::Init()
{
    if (m_viewportInitialized)
    {
        ...
        result = ms_pCubeSample->m_pd3d12Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_BUNDLE,
                                                                        __uuidof(ID3D12CommandAllocator),
                                                                        reinterpret_cast<void**>(&m_pBundleCommandAllocator));
...
void CubeSample::TopRightViewport::Draw()
{
    ...
    */
    ...
    ms_pCubeSample->GPA_BeginSample(m_gpaCmdListForBundle, m_bundleSampleId);                       // Start sample on the bundle
    #pragma endregion
    ...
    #pragma region GPA_CALL
    ms_pCubeSample->GPA_BeginCommandListForSampling(m_pd3d12CommandList, m_gpaCommandListId);       // Request GPA to start recording samples on the direct command list
    #pragma endregion
    ...
    #pragma region GPA_CALL
    ms_pCubeSample->GPA_CopyBundleSample(m_gpaCmdListForBundle, m_gpaCommandListId, tempVector);        // Copy the bundle sample to the direct command list after its execution
    #pragma endregion
...
bool CubeSample::BottomLeftViewport::Init()
{
    if (m_viewportInitialized)
    {
        ...
        result = ms_pCubeSample->m_pd3d12Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                                        __uuidof(ID3D12CommandAllocator),
                                                                        reinterpret_cast<void**>(&m_pSecondCmdListAllocator));
...
void CubeSample::BottomLeftViewport::Draw()
{
    ...
    */
    ...
    #pragma region GPA_CALL
    ms_pCubeSample->GPA_BeginCommandListForSampling(m_pd3d12CommandList, m_gpaCommandListId);       // Request GPA to start recording samples on the first direct command list
    ...
    #pragma region GPA_CALL
    ms_pCubeSample->GPA_BeginCommandListForSampling(m_pSecondCmdList, m_gpaCmdListForSecondCmdList);    // Request GPA to start recording samples on the second direct command list
    ...
    ms_pCubeSample->GPA_EndCommandListForSampling(m_gpaCmdListForSecondCmdList);                // End the second direct command list for sample recording before closing it.
    #pragma endregion
...
CubeSample::BottomRightViewport::BottomRightViewport():
    ...
    m_gpaCmdListForBundle(nullptr),
    m_bundleSampleId(0xFFFF),
    m_bundleSampleIdOnPrimaryCmdList(0xFFFF),
...
bool CubeSample::BottomRightViewport::Init()
{
    if (m_viewportInitialized)
    {
        ...
        result = ms_pCubeSample->m_pd3d12Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                                        __uuidof(ID3D12CommandAllocator),
                                                                        reinterpret_cast<void**>(&m_pSecondCommandListAllocator));
        ...
        result = ms_pCubeSample->m_pd3d12Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_BUNDLE,
                                                                        __uuidof(ID3D12CommandAllocator),
                                                                        reinterpret_cast<void**>(&m_pBundleCommandListAllocator));
...
void CubeSample::BottomRightViewport::Draw()
{
    ...
    */
    ...
    #pragma region GPA_CALL
    ms_pCubeSample->GPA_BeginCommandListForSampling(m_secondCommandList, m_gpaCmdListForSecondCmdList);     // Request GPA to start recording samples on the second direct command list
    #pragma endregion
    ...
    #pragma region GPA_CALL
    ms_pCubeSample->GPA_CopyBundleSample(m_gpaCmdListForBundle, m_gpaCommandListId, tempVector);            // Copy the bundle sample to the direct command list after its execution on first direct command list
    ms_pCubeSample->GPA_BeginSample(m_gpaCommandListId, m_sampleId);                                        // Start sample on the first direct command list i.e. m_gpaCommandListId
    ms_pCubeSample->GPA_ContinueSample(m_sampleId, m_gpaCmdListForSecondCmdList);                           // Continue the original sample to the second direct command list
    #pragma endregion
...
bool CubeSample::GPA_InitializeAndOpenContext()
{
    ...
    if (nullptr != m_pGpaFunctionTable)
    {
        ...
        success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_OpenContext(m_pd3d12Device, GPA_OPENCONTEXT_DEFAULT_BIT, &m_gpaContextId);
...
bool CubeSample::GPA_PopulateAvailableCounterInfo()
{
    bool success = true;

    if (nullptr != m_pGpaFunctionTable)
    {
        unsigned int numOfAvailableCounters = 0u;
        success = GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetNumCounters(m_gpaContextId, &numOfAvailableCounters);

        if (success)
        {
            for (unsigned int counterIter = 0; counterIter < numOfAvailableCounters; counterIter++)
            {
                CounterInfo counterInfo;
                counterInfo.m_counterIndex = counterIter;
                success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetCounterName(m_gpaContextId, counterIter, &counterInfo.m_counterName);
                success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetCounterDescription(m_gpaContextId, counterIter, &counterInfo.m_counterDescription);
                success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetCounterGroup(m_gpaContextId, counterIter, &counterInfo.m_counterGroup);
                success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetCounterUsageType(m_gpaContextId, counterIter, &counterInfo.m_usageType);
                success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetCounterUuid(m_gpaContextId, counterIter, &counterInfo.m_uuid);
                success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetCounterDataType(m_gpaContextId, counterIter, &counterInfo.m_counterDataType);


                if (success)
                {
                    m_availableCounters.push_back(counterInfo);
                }
            }
        }
    }
...
bool CubeSample::GPA_EnableAllCounters()
{
    bool success = true;
    ...
    if (nullptr != m_pGpaFunctionTable && m_profilingEnable)
    {
        if (nullptr != m_gpaSessionId)
        {
            ...
            if (success)
            {
                m_isAllCountersEnabled = true;
            }

            success = success && GPA_STATUS_OK == m_pGpaFunctionTable->GPA_GetPassCount(m_gpaSessionId,
                      &m_passRequired);
        }
        else
        {
...
bool CubeSample::GPA_CreateProfilingSession()
{
    ...
    if (nullptr != m_pGpaFunctionTable && m_profilingEnable)
    {
        ...
        {
            success = GPA_STATUS_OK == m_pGpaFunctionTable->GPA_CreateSession(m_gpaContextId,
                                                                              GPA_SESSION_SAMPLE_TYPE_DISCRETE_COUNTER,
                                                                              &m_gpaSessionId);
        }
        else
        {
...
bool CubeSample::GPA_BeginCommandListForSampling(ID3D12GraphicsCommandList* pGraphicsCmdList, GPA_CommandListId& gpaCmdId) const
{
    ...
    if (nullptr != m_pGpaFunctionTable && m_profilingEnable)
    {
        if (nullptr != m_gpaSessionId)
        {
            GPA_Command_List_Type cmdType = pGraphicsCmdList->GetType() == D3D12_COMMAND_LIST_TYPE_DIRECT ? GPA_COMMAND_LIST_PRIMARY : GPA_COMMAND_LIST_SECONDARY;
            success = GPA_STATUS_OK == m_pGpaFunctionTable->GPA_BeginCommandList(m_gpaSessionId,
                                                                                 m_currentPass,
                                                                                 pGraphicsCmdList,
                                                                                 cmdType,
                                                                                 &gpaCmdId);
...
bool CubeSample::GPA_CopyBundleSample(GPA_CommandListId pSecondaryGpaCmdList,
                                      ...
                                      std::vector<gpa_uint32> sampleIdList) const
{
    ...
    if (nullptr != m_pGpaFunctionTable && m_profilingEnable)
    {
        if (nullptr != pSecondaryGpaCmdList &&
            nullptr != pPrimaryGpaCmdList)
        {
            success = GPA_STATUS_OK == m_pGpaFunctionTable->GPA_CopySecondarySamples(pSecondaryGpaCmdList,
                      pPrimaryGpaCmdList,
                      static_cast<gpa_uint32>(sampleIdList.size()),
                      sampleIdList.data());
        }
    }
...
bool CubeSample::GPA_PopulateSessionResult()
{
    ...
    if (nullptr != m_pGpaFunctionTable && m_profilingEnable)
    {
        ...
        auto CollectSampleResult = [&](unsigned int sampleIndex)
        {
            ...
            for (CounterInfo counterInfo : m_availableCounters)
            {
                if (!m_isAllCountersEnabled &&
                    std::find(m_enabledCounters.cbegin(), m_enabledCounters.cend(), counterInfo.m_counterIndex) == m_enabledCounters.end())
                {
                    continue;
                }
                ...
                {
                    counterNamesHeader << "," << counterInfo.m_counterName;
                }

                if (GPA_DATA_TYPE_FLOAT64 == counterInfo.m_counterDataType)
                {
                    m_content << "," << *(reinterpret_cast<gpa_float64*>(pSampleResult) + counterIndex);
                }
                else if (GPA_DATA_TYPE_UINT64 == counterInfo.m_counterDataType)
                {
                    m_content << "," << *(reinterpret_cast<gpa_uint64*>(pSampleResult) + counterIndex);
                }
        ...
        if (m_counterDataFileStream.is_open())
        {
            ...
            {
                m_header << "Frame" << "," << "Wirerame" << "," << "Viewport" << "," << "Sample";
...
bool CubeSample::GPA_Log(const char* logMessage)
{
