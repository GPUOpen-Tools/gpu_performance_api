...
bool D3D11Triangle::Init()
{
    ...
    m_counterFileName = std::string(m_executablePath.begin(), m_executablePath.end()).append("DX11Triangle_counterData.csv");
    m_gpaLogFileName  = std::string(m_executablePath.begin(), m_executablePath.end()).append("DX11Triangle_gpaLog.txt");
    ...
    if (!dxgiAdapters.empty())
    {
        ...
        shaderPath.append(L"dx_11_triangle_shaders.hlsl");
...
bool D3D11Triangle::GPA_EndPass()
{
    ...
    if (nullptr != m_pGpaFunctionTable &&
        m_profilingEnable &&
        nullptr != m_gpaSessionId &&
        nullptr != m_gpaCommandListId)
    {
