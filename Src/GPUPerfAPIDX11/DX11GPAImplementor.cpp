        ...
        if (GPA_STATUS_OK == gpaStatus)
        {
            if (AMD_VENDOR_ID == adapterDesc.VendorId)
            {
                ...
                if (nullptr != hMonitor)
                {
                    ...
                    {
                        GPA_LogError("DX11 GPA Error: Unable to get hardware information.");
                    }
                }
                else
                {
                    GPA_LogError("DX11 GPA Error: Could not get device monitor description, hardware cannot be supported.");
                }
            }
            ...
            {
                GPA_LogError("DX11 GPA Error: Unknown device adapter");
            }
        }
        else
        {
            GPA_LogError("DX11 GPA Error: Unable to get adapter information");
        }
    }
    else
    {
        GPA_LogError("DX11 GPA Error: Unable to get device or either device feature level is not supported ");
    }
...
bool DX11GPAImplementor::CloseAPIContext(GPADeviceIdentifier pDeviceIdentifier, IGPAContext* pGpaContext)
{
    ...
    {
        delete reinterpret_cast<DX11GPAContext*>(pGpaContext);
    }
            ...
            if (S_OK == hr)
            {
                ...
                if (DxxExtUtils::IsMgpuPerfExtSupported(pExt))
                {
                    ...
                    if (nullptr == pExtPerfProfile)
                    {
                        ...
                        {
                            GPA_LogError("DX11 GPA Error: Unable to get perf counter extension for GCN device");
                        }
                    }
                    else
                    {
                        ...
                        {
                            GPA_LogError("DX11 GPA Error: No profilable GPU device available.");
                        }
                        else
                        {
                ...
                if (GPAUtil::GetCurrentModulePath(strDLLName))
                {
                    ...
                    {
                        GPA_LogError("DX11 GPA Error: Unable to load the get device info dll.");
                    }
                }
                else
                {
                    GPA_LogError("DX11 GPA Error: Unable to get the module path.");
                }
            }
            else
            {
                GPA_LogError("DX11 GPA Error: Unable to create DX11 extension.");
            }
        }
        else
        {
            GPA_LogError("DX11 GPA Error: Unable to initialize because extension creation is not available.");
        }
    }
    else
    {
#ifdef X64
        GPA_LogError("DX11 GPA Error: Unable to initialize because 'atidxx64.dll' is not available.");
#else
        GPA_LogError("DX11 GPA Error: Unable to initialize because 'atidxx32.dll' is not available.");
#endif
    }
