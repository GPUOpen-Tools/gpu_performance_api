...
namespace GPATools
{
    class CounterDefCMakeGenerator
    {
        ...
        public static void ProcessFiles(string fileName)
        {
            ...
            // PublicCounterDefs*
            {
                int indexOf = file.IndexOf((IsInternal ? Gpa.InternalStr : Gpa.PublicStr) + Gpa.CounterDefOutFileName, StringComparison.Ordinal);
                if (indexOf != -1)
                {
            ...
            // DerivedCounterDefs*
            {
                int indexOf = file.IndexOf((IsInternal ? Gpa.InternalStr : Gpa.PublicStr) + Gpa.DerivedCounterOutFileName, StringComparison.Ordinal);
                if (indexOf != -1)
                {
                    string api = Gpa.GetApiFromFileName(file);
                    if (null != api)
