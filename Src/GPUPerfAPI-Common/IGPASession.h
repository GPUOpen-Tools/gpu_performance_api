...
enum GPASessionState : uint8_t
{
    ...
    GPA_SESSION_STATE_COMPLETED,                ///< Session's results are available.
};
...
public:
    ...
    /// Begins a session
    /// \return true if session started successfully otherwise false
    ...
    /// \return true if command list exist in the session otherwise false
    ...
    /// Updates the specified pass's results cache if new results are available.
    ...
    /// Checks whether the session is running or not
