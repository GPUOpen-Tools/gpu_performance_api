...
ROCmGPASample::ROCmGPASample(GPAPass* pPass,
...
hsa_status_t ROCmGPASample::DispatchCallback(const rocprofiler_callback_data_t* pCallbackData,
                                             void* pUserData,
                                             rocprofiler_group_t* pGroup)
{
    ...
    else
    {
        ...
        status = pROCProfilerModule->rocprofiler_open(pCallbackData->agent, pDispatchData->m_pFeatures, pDispatchData->m_featureCount, &pContext, 0, &properties);
