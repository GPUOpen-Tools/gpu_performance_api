...
inline void UpdateAsicSpecificCounters(GDT_HW_GENERATION desiredGeneration, GDT_HW_ASIC_TYPE asicType, GPA_DerivedCounters& counters)
{
    ...
    if (VKGfx8_Baffin::UpdateAsicSpecificCounters(desiredGeneration, asicType, counters))
    {
        return;
    }

    if (VKGfx8_Carrizo::UpdateAsicSpecificCounters(desiredGeneration, asicType, counters))
    {
        return;
    }

    if (VKGfx8_Ellesmere::UpdateAsicSpecificCounters(desiredGeneration, asicType, counters))
    {
        return;
    }

    if (VKGfx8_Fiji::UpdateAsicSpecificCounters(desiredGeneration, asicType, counters))
    {
        return;
    }

    if (VKGfx8_Iceland::UpdateAsicSpecificCounters(desiredGeneration, asicType, counters))
    {
        return;
    }

    if (VKGfx8_Tonga::UpdateAsicSpecificCounters(desiredGeneration, asicType, counters))
    {
        return;
    }
