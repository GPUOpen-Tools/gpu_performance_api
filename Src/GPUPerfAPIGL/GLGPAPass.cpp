...
bool GLGPAPass::EndSample(IGPACommandList* pGpaCmdList)
{
    bool retVal = false;
...
bool GLGPAPass::InitializeCounters(const GLPerfMonitorId& glPerfMonitorId)
{
    ...
    auto EnableCounter = [&](const CounterIndex & counterIndex) ->bool
    {
        ...
        {
            _oglSelectPerfMonitorCountersAMD(glPerfMonitorId,
            GL_TRUE,
        ...
        else
        {
            ...
            {
                _oglGetPerfMonitorCounterStringAMD(pCounter->m_groupIdDriver,
...
{
    // Create perf monitor
    _oglGenPerfMonitorsAMD(1, &m_glPerfMonitorId);
...
bool GLGPAPass::GLPerfMonitor::Clear(bool forceClear)
{
    ...
    _oglDeletePerfMonitorsAMD(1, &m_glPerfMonitorId);
