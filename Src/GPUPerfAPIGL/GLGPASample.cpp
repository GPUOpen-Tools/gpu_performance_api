...
GLGPASample::GLGPASample(GPAPass* pPass,
...
{
    bool isDataCollected = IsComplete();
...
bool GLGPASample::BeginRequest()
{
    ...
    {
        // handle timing pass
        _oglQueryCounter(m_sampleDataBuffer.m_gpuTimeQuery[0], GL_TIMESTAMP);
    ...
    else
    {
        ...
        {
            _oglBeginPerfMonitorAMD(m_sampleDataBuffer.m_glPerfMonitorId);

            if (!oglUtils::CheckForGLError("Unable to begin the GL hardware counters perf monitor."))
            {
                success = true;
            }
        }
...
bool GLGPASample::EndRequest()
{
    ...
    {
        // handle timing pass
        _oglQueryCounter(m_sampleDataBuffer.m_gpuTimeQuery[1], GL_TIMESTAMP);
    ...
    else
    {
        ...
        _oglEndPerfMonitorAMD(m_sampleDataBuffer.m_glPerfMonitorId);
...
bool GLGPASample::CopyResults()
{
    ...
    if (m_pGlGpaPass->IsTimingPass())
    {
        ...
        _oglGetQueryObjectui64vEXT(m_sampleDataBuffer.m_gpuTimeQuery[0], GL_QUERY_RESULT, &gpuTimeResults[0]);

        if (!oglUtils::CheckForGLError("Unable to get first timing data."))
        {
            _oglGetQueryObjectui64vEXT(m_sampleDataBuffer.m_gpuTimeQuery[1], GL_QUERY_RESULT, &gpuTimeResults[1]);
            ...
            if (!oglUtils::CheckForGLError("Unable to get second timing data."))
            {
                ...
                memcpy(pSampleResult->m_pResultBuffer, &timingDifference, sizeof(gpa_uint64));
                success = true;
            }
        }
    }
    else
    {
        ...
        _oglGetPerfMonitorCounterDataAMD(perfMonitorId,
                ...
                if (!oglUtils::CheckForGLError("Unable to get the counter data size."))
                {
                    ...
                    if (!oglUtils::CheckForGLError("Unable to get the counter data."))
                    {
                        ...
                        for (CounterCount counterCountIter = 0; counterCountIter < counterCount; counterCountIter++)
                        {
                            ...
                            {
                                GLuint* pDest = reinterpret_cast<GLuint*>(&pSampleResult->m_pResultBuffer[curCounterResultIndex]);
                                pDest[0] = 0;
                                pDest[1] = 0;
...
bool GLGPASample::CreateGPUTimeQuery()
{
    ...
    _oglGenQueries(2, m_sampleDataBuffer.m_gpuTimeQuery);
...
bool GLGPASample::DeleteGPUTimeQueries()
{
    ...
    {
        _oglDeleteQueries(1, &m_sampleDataBuffer.m_gpuTimeQuery[0]);
        success = !oglUtils::CheckForGLError("Unable to delete the query.");
    }

    if (success &&
        m_sampleDataBuffer.m_gpuTimeQuery[1] != 0)
    {
        _oglDeleteQueries(1, &m_sampleDataBuffer.m_gpuTimeQuery[1]);
        success = !oglUtils::CheckForGLError("Unable to delete the query.");
    }
