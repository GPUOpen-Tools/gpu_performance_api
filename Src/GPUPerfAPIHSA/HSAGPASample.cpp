...
bool HSAGPASample::BeginRequest()
{
    ...
    if (nullptr != pHsaToolsRTModule && pHsaToolsRTModule->IsModuleLoaded())
    {
        hsa_ext_tools_pre_dispatch_callback_function preDispatchCallBackFunc = nullptr;
        hsa_ext_tools_pre_dispatch_callback_function postDispatchCallBackFunc = nullptr;

        GPA_HSA_Context gpaHsaContext = m_pHSAGpaContext->HSAContext();
        hsa_queue_t* pQueue = const_cast<hsa_queue_t*>(gpaHsaContext.m_pQueue);
        hsa_status_t hsaStatus = pHsaToolsRTModule->ext_tools_get_callback_functions(pQueue, preDispatchCallBackFunc, postDispatchCallBackFunc);

        if (HSA_STATUS_SUCCESS == hsaStatus)
        {
            if (preDispatchCallBackFunc == HSA_PreDispatchCallback &&
                postDispatchCallBackFunc == HSA_PostDispatchCallback)
            {
                if (nullptr != m_pHSAGpaContext &&
                    nullptr == m_pHSAGpaContext->GetRunningHSASample())
                {
                    success = m_pHSAGpaContext->SetRunningHSASample(this);
                }
            }
            else
            {
                success = BeginInternalRequest();
            }
        }
    }
...
bool HSAGPASample::BeginInternalRequest()
{
    bool success = false;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        // Begin
                        status = pHsaToolsRTModule->ext_tools_pmu_begin(pmu, m_pHSAGpaContext->HSAContext().m_pQueue, m_pHSAGpaContext->HSAContext().m_pAqlTranslationHandle, true);
...
bool HSAGPASample::EndInternalRequest() const
{
    ...
    {
        hsa_status_t status = HSAToolsRTModuleLoader::Instance()->GetAPIRTModule()->ext_tools_pmu_end(pmu, m_pHSAGpaContext->HSAContext().m_pQueue, m_pHSAGpaContext->HSAContext().m_pAqlTranslationHandle);
